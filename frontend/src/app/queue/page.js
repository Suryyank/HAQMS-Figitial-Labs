...
import QueueHeader from "@/features/dashboard/components/QueueHeader";
import { useQueue } from "@/features/queue/hooks/useQueue";

export default function QueueMonitor() {
  const { groupedTokens, loading, error, refreshCount } = useQueue();
        ...
        {loading && Object.keys(groupedTokens).length === 0 ? (
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {Object.entries(groupedTokens).map(([docId, docInfo]) => (
              <div key={docId} className="glass rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-full hover:shadow-teal-500/5 hover:border-teal-500/30 transition-all duration-300">
                    ...
                    {docInfo.waiting.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {docInfo.waiting.map((token) => (
                          <div key={token.id} className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300" title={`Patient: ${token.patient.name}`}>
                            #{token.tokenNumber}
                          </div>
                        ))}
                      </div>
