...
router.post('/register', async (req, res) => {
  try {
    // SENSITIVE CONSOLE LOG REMOVED: Never log raw request bodies with passwords in plain text!
    console.log('[DEBUG] Registering user (email logged only):', req.body.email);
...
router.post('/login', async (req, res) => {
  try {
    // SENSITIVE CONSOLE LOG REMOVED: Do not log plain-text passwords on login attempts!
    ...
    // Weak JWT token generation fixed: signed token with 1d expiration limit
