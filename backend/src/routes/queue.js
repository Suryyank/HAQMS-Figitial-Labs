...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    // Run within a transaction and utilize row-level locking (FOR UPDATE) on Doctor to prevent race conditions.
    const newToken = await prisma.$transaction(async (tx) => {
      // 1. Lock the Doctor row to serialize concurrent checks for the same doctor
      ...
      // Removed performance/concurrency bug (artificial sleep removed entirely)
